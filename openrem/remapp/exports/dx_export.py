...
def exportDX2excel(filterdict):
    ...
    for i, exams in enumerate(e):
        examdata = [
            ...
            exams.projectionxrayradiationdose_set.get().accumulated_xray_dose_set.get().accumprojxraydose_set.get().convert_gym2_to_cgycm2(),
            ]
...
def dxxlsx(filterdict):
    ...
    numcolumns = (22 * max_events['projectionxrayradiationdose__accumulated_xray_dose__accumprojxraydose__total_number_of_radiographic_frames__max']) + 14 - 1
    numrows = e.count()
    ...
    for row,exams in enumerate(e):
        ...
        examdata = [
            ...
            exams.study_description,
            exams.requested_procedure_code_meaning,
            str(exams.projectionxrayradiationdose_set.get().accumulated_xray_dose_set.get().accumprojxraydose_set.get().total_number_of_radiographic_frames),
            str(exams.projectionxrayradiationdose_set.get().accumulated_xray_dose_set.get().accumprojxraydose_set.get().convert_gym2_to_cgycm2()),
			]
        for s in exams.projectionxrayradiationdose_set.get().irradiation_event_xray_data_set.all():
            examdata += [
                s.acquisition_protocol,

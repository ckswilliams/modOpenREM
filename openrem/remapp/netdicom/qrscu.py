...
def qrscu(
    ...
    Args:
        qr_scp_pk (int): Database ID/pk of the remote QR SCP
        store_scp_pk (int): Database ID/pk of the local store SCP
        implicit (bool): Prefer implicit transfer syntax (preference possibly not implemented)
        explicit (bool): Prefer explicit transfer syntax (preference possibly not implemented)
        move (bool): Automatically trigger move request when query is complete
        query_id (str): UID of query if generated by web interface
        date_from (str): Date to search from, format yyyy-mm-dd
        date_until (str): Date to search until, format yyyy-mm-dd
        modalities (list): Modalities to search for, options are CT, MG, DX and FL
        inc_sr (bool): Include studies that only have structured reports in (unknown modality)
        duplicates (bool): If True, studies that already exist in the database are removed from the query results
        *args:
        **kwargs:

...
def _rsdr2db(dataset):
    ...
    if g.modality_type == 'RF':
        ...
        if send_alert_emails:
            ...
            html_msg_body += '<tr><td>Institution:</td><td>{0}</td></tr>'.format(g.generalequipmentmoduleattr_set.get().institution_name)
            html_msg_body += '<tr><td>Display name:</td><td>{0}</td></tr>'.format(g.generalequipmentmoduleattr_set.get().unique_equipment_name.display_name)
            ...
            if calc_accum_dose_over_delta_weeks_on_import:
                try:
                    if accum_int_proj_to_update.total_dap_delta_gym2_to_cgycm2() >= alert_values['alert_total_dap_rf'] or accum_int_proj_to_update.dose_rp_total_over_delta_weeks >= alert_values['alert_total_rp_dose_rf']:
                        ...
                        html_msg_body += '<p>Total DAP and dose and RP for this patient ID from the past {0} weeks</p>'.format(week_delta)
                        html_msg_body += '<table>'
                        html_msg_body += '<tr><th>Item</th><th>Value</th><th>Comments</th></tr>'
                        html_msg_body += '<tr><td>Number of studies from the past {0} weeks:</td><td>{1}</td><td></td></tr>'.format(week_delta, included_studies.count())
                        html_msg_body += '<tr><td>Total DAP from the past {0} weeks (cGy.cm2):</td><td>{1:.1f}</td>'.format(week_delta, accum_int_proj_to_update.total_dap_delta_gym2_to_cgycm2())
                        if accum_int_proj_to_update.total_dap_delta_gym2_to_cgycm2() >= alert_values['alert_total_dap_rf']:
                            ...
                            html_msg_body += '<td>Above the {1:.1f} alert level</td></tr>'.format(week_delta, alert_values['alert_total_dap_rf'])
                        else:
                            msg_body += '\r\n'
                        ...
                        html_msg_body += '<tr><td>Total dose at RP from the past {0} weeks (Gy)</td><td>{1:.1f}</td>'.format(week_delta, accum_int_proj_to_update.dose_rp_total_over_delta_weeks)
                        if accum_int_proj_to_update.dose_rp_total_over_delta_weeks >= alert_values['alert_total_rp_dose_rf']:
                            ...
                            html_msg_body += '<td>Above the {1:.1f} alert level</td></tr>'.format(week_delta, alert_values[ 'alert_total_rp_dose_rf'])
                        else:
                            msg_body += '\r\n'
                            html_msg_body += '<td>None</td></tr>'
                        msg_body += '\r\n'
                        ...
                        html_msg_body += '<p>All studies for this patient ID from the past {0} weeks are:</p>'.format(week_delta)
                        html_msg_body += '<table>'
                        ...
                        for linked_study in linked_studies:
                            ...
                            html_msg_body += '<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4:.1f}</td><td>{5:.1f}</td></tr>'.format(reverse('rf_detail_view', args=[linked_study.pk]), linked_study.study_date.strftime('%x'), linked_study.study_time.strftime('%X'), linked_study.accession_number, linked_study_dap, linked_study_rp_dose)
                        html_msg_body += '</table>'
